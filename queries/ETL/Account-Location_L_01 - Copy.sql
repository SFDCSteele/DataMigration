...
LEFT JOIN sfdc.Account_Account_L_02 AS B-- for OwnerId, Account__c -- VERIFY SOURCE FRESHNESS !!!
ON A.AIMSAccount__c = B.AIMSAccount__c

LEFT JOIN sfdc.[Id_User_fullData_230424-1346] AS C -- for CreatedById -- VERIFY SOURCE FRESHNESS !!!
ON TRIM(A.CreatedById) = TRIM(C.Alias) AND TRIM(C.Alias) IS NOT NULL AND C.title <> 'MOCK-01'

LEFT JOIN sfdc.[Id_User_fullData_230424-1346] AS D -- for LastModifiedById -- VERIFY SOURCE FRESHNESS !!!
ON TRIM(A.LastModifiedById) = TRIM(D.Alias) AND TRIM(D.Alias) IS NOT NULL AND D.title <> 'MOCK-01'

LEFT JOIN sfdc.[Id_User_fullData_230424-1346] AS E -- for StatusUpdateUserid__c -- VERIFY SOURCE FRESHNESS !!!
ON TRIM(A.StatusUpdateUserid__c) = TRIM(E.Alias) AND TRIM(E.Alias) IS NOT NULL AND E.title <> 'MOCK-01'
