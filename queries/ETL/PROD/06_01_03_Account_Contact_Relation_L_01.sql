...
Where C.Id Is NOT NULL AND D.Id Is NOT NULL 
		...
		AND E.ID is not null AND A.Primary__c = 'true' AND A.RelationshipDirection__c = 'CONTACT_TO_ACCOUNT' --comment this is for updating the primary (record Id is NOT null)
		--AND E.ID is null AND A.Primary__c = 'false' AND A.RelationshipDirection__c = 'CONTACT_TO_ACCOUNT' --comment this is for inserting the secondary (record Id is null)
